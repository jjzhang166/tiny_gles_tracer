...
#define eglCreatePlatformWindowSurfaceEXT_  \
    ((EGLSurface (*)(EGLDisplay dpy, EGLConfig config, void *native_window, const EGLint *attrib_list))  \
    EGL_ENTRY_PTR(eglCreatePlatformWindowSurfaceEXT_Idx))
...
eglCreatePlatformWindowSurfaceEXT (EGLDisplay dpy, EGLConfig config, void *native_window, const EGLint *attrib_list)
{
    ...
    fprintf (g_log_fp, "eglCreatePlatformWindowSurfaceEXT(%p, %p, %p, %p);", dpy, config, native_window, attrib_list);
    ...
    if (eglCreatePlatformWindowSurfaceEXT_)
    {
        sfc = eglCreatePlatformWindowSurfaceEXT_ (dpy, config, native_window, attrib_list);
