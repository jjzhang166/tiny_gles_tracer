...
#define eglCreatePlatformPixmapSurfaceEXT_  \
    ((EGLSurface (*)(EGLDisplay dpy, EGLConfig config, void *native_window, const EGLint *attrib_list))  \
    EGL_ENTRY_PTR(eglCreatePlatformPixmapSurfaceEXT_Idx))
...
eglCreatePlatformPixmapSurfaceEXT (EGLDisplay dpy, EGLConfig config, void *native_pixmap, const EGLint *attrib_list)
{
    ...
    fprintf (g_log_fp, "eglCreatePlatformPixmapSurfaceEXT(%p, %p, %p, %p);", dpy, config, native_pixmap, attrib_list);
    ...
    if (eglCreatePlatformPixmapSurfaceEXT_)
    {
        sfc = eglCreatePlatformPixmapSurfaceEXT_ (dpy, config, native_pixmap, attrib_list);
