...
glDrawElementsInstancedBaseVertex (GLenum mode, GLsizei count, GLenum type, const void *indices,
                                   GLsizei instancecount, GLint basevertex)
{
    prepare_gles_tracer ();

    fprintf (g_log_fp, "glDrawElementsInstancedBaseVertex(%s, %d, %s, %p, %d, %d); // ",
             get_mode_str (mode), count, get_type_str (type), indices, instancecount, basevertex);
    ...
    glDrawElementsInstancedBaseVertex_ (mode, count, type, indices, instancecount, basevertex);

    if (g_capture_on_draw)
    {
        char fname[128];
        snprintf (fname, sizeof(fname), "%05d_glDrawElementsInstancedBaseVertex_%05d_%05d.tga", g_pid, g_frame_counter, g_draw_counter);
