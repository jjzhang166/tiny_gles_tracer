...
glDrawRangeElementsBaseVertex (GLenum mode, GLuint start, GLuint end, GLsizei count, GLenum type,
                               const void *indices, GLint basevertex)
{
    prepare_gles_tracer ();

    fprintf (g_log_fp, "glDrawRangeElementsBaseVertex(%s, %d, %d, %d, %s, %p, %d); // ",
             get_mode_str (mode), start, end, count, 
             get_type_str (type), indices, basevertex);
    ...
    glDrawRangeElementsBaseVertex_ (mode, start, end, count, type, indices, basevertex);

    if (g_capture_on_draw)
    {
        char fname[128];
        snprintf (fname, sizeof(fname), "%05d_glDrawRangeElementsBaseVertex_%05d_%05d.tga", g_pid, g_frame_counter, g_draw_counter);
