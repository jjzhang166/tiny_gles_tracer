...
glDrawElementsBaseVertex (GLenum mode, GLsizei count, GLenum type, const void *indices, GLint basevertex)
{
    ...
    fprintf (g_log_fp, "glDrawElementsBaseVertex(%s, %d, %s, %p, %d); // ",
             get_mode_str (mode), count, get_type_str (type), indices, basevertex);
    ...
    glDrawElementsBaseVertex_ (mode, count, type, indices, basevertex);

    if (g_capture_on_draw)
    {
        char fname[128];
        snprintf (fname, sizeof(fname), "%05d_glDrawElementsBaseVertex_%05d_%05d.tga", g_pid, g_frame_counter, g_draw_counter);
