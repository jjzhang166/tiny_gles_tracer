...
glShaderBinary (GLsizei count, const GLuint *shaders, GLenum binaryformat, const void *binary, GLsizei length)
{
    ...
    fprintf (g_log_fp, "glShaderBinary(%d, %p, %s, %p, %d);\n", 
             count, shaders, get_binary_name_str (binaryformat), binary, length);
    glShaderBinary_ (count, shaders, binaryformat, binary, length);
}
