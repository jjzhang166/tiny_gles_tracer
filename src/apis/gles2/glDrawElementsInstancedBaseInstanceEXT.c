...
#define glDrawElementsInstancedBaseInstanceEXT_         \
    ((void (*)(GLenum mode, GLsizei count, GLenum type, const void *indices, GLsizei instancecount,  GLuint baseinstance))   \
    GLES_ENTRY_PTR(glDrawElementsInstancedBaseInstanceEXT_Idx))
...
glDrawElementsInstancedBaseInstanceEXT (GLenum mode, GLsizei count, GLenum type, const void *indices, GLsizei instancecount,  GLuint baseinstance)
{
    ...
    fprintf (g_log_fp, "glDrawElementsInstancedBaseInstanceEXT(%s, %d, %s, %p, %d, %d); // ",
             get_mode_str (mode), count, 
             get_type_str (type), indices, instancecount, baseinstance);
    ...
    glDrawElementsInstancedBaseInstanceEXT_ (mode, count, type, indices, instancecount, baseinstance);

    if (g_capture_on_draw)
    {
        char fname[128];
        snprintf (fname, sizeof(fname), "%05d_glDrawElementsInstancedBaseInstanceEXT_%05d_%05d.tga", g_pid, g_frame_counter, g_draw_counter);
