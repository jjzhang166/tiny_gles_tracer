...
#define glCompressedTexImage2D_   \
    ...
    GLES_ENTRY_PTR(glCompressedTexImage2D_Idx))


GL_APICALL void GL_APIENTRY
glCompressedTexImage2D (GLenum target, GLint level, GLenum internalformat, GLsizei width, GLsizei height, 
                        GLint border, GLsizei imageSize, const void *data)
{
    ...
    fprintf (g_log_fp, "glCompressedTexImage2D(%s, %d, %s, %d, %d, %d, %d, %p);",
                get_tex_target_str (target), level,
                get_tex_format_str (internalformat), width, height, border,
                imageSize, data);
