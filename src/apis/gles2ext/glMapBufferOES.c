...
#define glMapBufferOES_   \
    ((void * (*)(GLenum target, GLenum access))  \
    GLES_ENTRY_PTR(glMapBufferOES_Idx))
...
glMapBufferOES (GLenum target, GLenum access)
{
    ...
    void *ret = glMapBufferOES_ (target, access);

    fprintf (g_log_fp, "glMapBufferOES(%s, 0x%x); // ret=%p\n",
             get_target_str (target), access, ret);
