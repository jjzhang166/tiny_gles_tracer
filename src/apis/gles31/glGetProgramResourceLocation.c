...
#define glGetProgramResourceLocation_   \
    ((GLint (*)(GLuint program, GLenum programInterface, const GLchar *name))  \
    GLES_ENTRY_PTR(glGetProgramResourceLocation_Idx))
...
glGetProgramResourceLocation (GLuint program, GLenum programInterface, const GLchar *name)
{
    ...
    GLuint ret = glGetProgramResourceLocation_ (program, programInterface, name);

    fprintf (g_log_fp, "glGetProgramResourceLocation(%d, %s, %s); // ret=%d\n",
