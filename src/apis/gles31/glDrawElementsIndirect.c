...
#define glDrawElementsIndirect_         \
    ((void (*)(GLenum mode, GLenum type, const void *indirect))   \
    GLES_ENTRY_PTR(glDrawElementsIndirect_Idx))
...
glDrawElementsIndirect (GLenum mode, GLenum type, const void *indirect)
{
    ...
    fprintf (g_log_fp, "glDrawElementsIndirect(%s, %s, %p); // ",
             get_mode_str (mode), get_type_str (type), indirect);
    ...
    glDrawElementsIndirect_ (mode, type, indirect);

    if (g_capture_on_draw)
    {
        char fname[128];
        snprintf (fname, sizeof(fname), "%05d_glDrawElementsIndirect_%05d_%05d.tga", g_pid, g_frame_counter, g_draw_counter);
