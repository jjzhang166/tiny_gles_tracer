...
#define glDrawArraysInstanced_         \
    ((void (*)(GLenum mode, GLint first, GLsizei count, GLsizei instancecount))   \
    GLES_ENTRY_PTR(glDrawArraysInstanced_Idx))
...
glDrawArraysInstanced (GLenum mode, GLint first, GLsizei count, GLsizei instancecount)
{
    ...
    fprintf (g_log_fp, "glDrawArraysInstanced(%s, %d, %d, %d); // ",
             get_mode_str (mode), first, count, instancecount);
    ...
    glDrawArraysInstanced_ (mode, first, count, instancecount);

    if (g_capture_on_draw)
    {
        char fname[128];
        snprintf (fname, sizeof(fname), "%05d_glDrawArraysInstanced_%05d_%05d.tga", g_pid, g_frame_counter, g_draw_counter);
