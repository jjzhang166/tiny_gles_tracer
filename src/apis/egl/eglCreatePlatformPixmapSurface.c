...
eglCreatePlatformPixmapSurface (EGLDisplay dpy, EGLConfig config, void *native_pixmap, const EGLAttrib *attrib_list)
{
    ...
    fprintf (g_log_fp, "eglCreatePlatformPixmapSurface(%p, %p, %p, %p);", dpy, config, native_pixmap, attrib_list);
    ...
    if (eglCreatePlatformPixmapSurface_)
    {
        sfc = eglCreatePlatformPixmapSurface_ (dpy, config, native_pixmap, attrib_list);
