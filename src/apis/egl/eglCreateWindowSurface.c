...
#define eglCreateWindowSurface_  \
    ((EGLSurface (*)(EGLDisplay dpy, EGLConfig config, EGLNativeWindowType win, const EGLint *attrib_list))  \
    EGL_ENTRY_PTR(eglCreateWindowSurface_Idx))
...
eglCreateWindowSurface (EGLDisplay dpy, EGLConfig config, EGLNativeWindowType win, const EGLint *attrib_list)
{
    ...
    fprintf (g_log_fp, "eglCreateWindowSurface(%p, %p, %ld, %p);", dpy, config, win, attrib_list);
    ...
    if (eglCreateWindowSurface_)
    {
        sfc = eglCreateWindowSurface_ (dpy, config, win, attrib_list);
