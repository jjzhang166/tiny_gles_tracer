...
#define eglCreatePixmapSurface_  \
    ((EGLSurface (*)(EGLDisplay dpy, EGLConfig config, EGLNativePixmapType pixmap, const EGLint *attrib_list))  \
    EGL_ENTRY_PTR(eglCreatePixmapSurface_Idx))
...
eglCreatePixmapSurface (EGLDisplay dpy, EGLConfig config, EGLNativePixmapType pixmap, const EGLint *attrib_list)
{
    ...
    fprintf (g_log_fp, "eglCreatePixmapSurface(%p, %p, %ld, %p);", dpy, config, pixmap, attrib_list);
    ...
    if (eglCreatePixmapSurface_)
    {
        sfc = eglCreatePixmapSurface_ (dpy, config, pixmap, attrib_list);
